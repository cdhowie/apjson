...
mod queson {
    ...
    /// Deserialize a JSON-encoded value from a file-like object.
    ///
    /// Note that this function is currently a shim that reads the entire input
    /// into memory.  This may be changed later.
    ///
    /// `object_hook` can be set to a function.  Whenever a JSON object has been
    /// fully deserialized, this function will be called with the resulting
    /// `dict` as its only parameter.  The return value of the function (which
    ...
    /// untrusted.
    #[pyfunction]
    #[pyo3(signature = (file, *, object_hook = None, depth_limit = None))]
    fn load<'py>(
        file: &Bound<'py, PyAny>,
        object_hook: Option<&'py Bound<'py, PyFunction>>,
        depth_limit: Option<NonZeroUsize>,
        ...
        loads(
            &file.call_method0(pyo3::intern!(file.py(), "read"))?,
            object_hook,
            depth_limit,
        )
    }
    ...
    /// Serialize a value into file-like object.
    ///
    /// Note this function is currently a shim that buffers the output in memory
    /// and writes it all at once.  This may change later.
    ///
    /// To maintain compatilibity with `json.dump`, this function writes a
    /// `str`. This is less efficient than writing a `bytes`.
    ///
    /// The following Python types are supported:
    ///
    /// * `None` becomes `null`.
    ...
    /// disabled, no significant performance difference was detected.
    #[pyfunction]
    #[pyo3(signature = (value, file, *, object_hook = None, check_circular = true))]
    fn dump<'py>(
        value: Bound<'py, PyAny>,
        file: Bound<'py, PyAny>,
        object_hook: Option<&'py Bound<'py, PyFunction>>,
        ...
        let json = dumps(value, object_hook, check_circular)?;

        file.call_method1(pyo3::intern!(file.py(), "write"), (json,))?;
